<?PHP
	...
	$pdf ->Cell(0,10,'Laporan Rekapitulasi Detail',0,0,'C');
	$pdf ->Ln(25);
	$pdf ->SetX(20);
	...
	$pdf ->Cell(100,15,"Tanggal Sewa",1,"LR","C", true);
	$pdf ->Cell(100,15,"Tanggal Kembali",1,"LR","C", true);
	$pdf ->Cell(90,15,"Total Harga",1,"LR","C", true);
	$pdf ->Cell(130,15,"Keterangan",1,"LR","C", true);
	$pdf ->Ln();
	$tampil=$koneksi->query("SELECT * FROM tbdetailsewa LEFT JOIN tbcostumer ON tbdetailsewa.kd_cs=tbcostumer.kd_cs LEFT JOIN tbsewa ON tbdetailsewa.kd_sewa=tbsewa.kd_sewa LEFT JOIN tbmobil ON tbsewa.no_polisi=tbmobil.no_polisi LEFT JOIN tbmerk ON tbmobil.kd_merk=tbmerk.kd_merk LEFT JOIN tbsopir ON tbdetailsewa.kd_sopir=tbsopir.kd_sopir ORDER BY kd_detail ASC");
	$nilaiY = $pdf->GetY();
	while ($row = $tampil->fetch_object()){
		;
	$pdf ->SetX(20);
	$pdf ->Cell(20,40,$row->kd_detail,1,"LR","C");
	...
	$pdf ->Cell(100,40,$row->tgl_sewa,1,"LR","C");
	$pdf ->Cell(100,40,$row->tgl_kembali,1,"LR","C");
	$pdf ->Cell(90,40,$row->total_harga,1,"LR","C");
	$pdf ->Cell(130,40,$row->ket,1,"LR","C");
	$pdf ->Ln();
	$nilaiY = $pdf->GetY();
	}

	$pdf ->Output('Rekap-Detail-'.date('dFY').'.pdf','I');
